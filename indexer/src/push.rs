...
use indexer_db::push::{DeviceRegistrationPartition, WatchedAddressPartition, WatchedGroupIdPartition};
...
impl PushRegistry {
    ...
    pub fn register(
        ...
        validate_registration_limits(&watched_addresses, &watched_group_ids, &aliases)?;
            ...
            if !group_ids_unchanged {
                ...
                for group_id in &existing.watched_group_ids {
                    if !new_set.contains(group_id.as_str()) {
                        if let Ok(bytes) = decode_group_id_hex(group_id) {
    ...
    pub fn tokens_for_group(&self, blinded_group_id: &[u8; 32]) -> anyhow::Result<Vec<String>> {
        self.metrics.increment_db_read_ops_total(1);
        let db_read_started = Instant::now();
        let result = (|| {
            let rtx = self.tx_keyspace.read_tx();
            ...
            for entry in self
                .watched_group_partition
                .get_by_blinded_group_id_prefix(&rtx, blinded_group_id)
            {
                let key = entry?;
                if let Some(token) = token_from_watched_group_key_bytes(key.as_ref()) {
                    tokens.push(token);
                }
            }
            Ok(tokens)
    ...
    pub fn prune_group_id_watchers(&self, blinded_group_id: &[u8; 32]) -> anyhow::Result<()> {
        self.metrics.increment_db_read_ops_total(1);
        let db_read_started = Instant::now();
        let rtx = self.tx_keyspace.read_tx();
        let keys = self
    ...
    pub fn all_registered_tokens(&self) -> anyhow::Result<Vec<String>> {
        self.metrics.increment_db_read_ops_total(1);
        let db_read_started = Instant::now();
        let result = (|| {
            let rtx = self.tx_keyspace.read_tx();
            let mut tokens = Vec::new();
            for entry in self.device_partition.iter_tokens_rtx(&rtx) {
                let key = entry?;
                if let Ok(token) = std::str::from_utf8(key.as_ref()) {
    ...
    /// Group messages have no on-chain recipient address, so matching is purely by
    /// `blinded_group_id` - no alias/primary filtering applies (those concepts are
    /// address-based).
    fn matching_tokens_for_group(
        &mut self,
        blinded_group_id: &[u8; 32],
    ) -> anyhow::Result<Vec<String>> {
        let tokens = self.tokens_for_group(blinded_group_id)?;
        self.metrics
            .increment_push_tokens_looked_up_total(tokens.len() as u64);
        if tokens.is_empty() {
            self.prune_group_id_watchers(blinded_group_id)?;
        }
        Ok(tokens)
    }

    /// `gctl` control messages are sent from the admin's own address. If nobody is watching
    /// that address yet (a brand-new member has no reason to be watching an admin they've
    /// never interacted with before), fan out to every registered device instead of dropping
    /// the event - each device attempts its own ECIES decrypt and only the real target
    /// succeeds.
    ...
    pub fn process(mut self) {
        ...
        while let Ok(command) = self.commands.recv() {
            match command {
                ...
                PushRegistryCommand::MatchGroupTokens {
                    blinded_group_id,
                    response,
                } => {
                    let result = self.registry.matching_tokens_for_group(&blinded_group_id);
                    let _ = response.send(result);
                }
                PushRegistryCommand::MatchGroupControlTokens {
                    ...
                    let result = self
                        .registry
                        .matching_tokens_for_group_control(&watched_address);
                    let _ = response.send(result);
                }
            }
        }
...
impl PushRegistryHandle {
    ...
    pub async fn matching_tokens_for_group(
        ...
        self.request(|response| PushRegistryCommand::MatchGroupTokens {
            blinded_group_id,
            response,
        })
        .await
...
impl PushDispatcher {
    ...
    async fn handle_event(&mut self, event: PushEvent) -> anyhow::Result<()> {
        ...
        let tokens = match event.kind {
            PushEventKind::GroupMessage => {
                let Some(blinded_group_id) = event.blinded_group_id else {
                    return Ok(());
                };
                self.registry
                    .matching_tokens_for_group(blinded_group_id)
                    .await?
            }
            PushEventKind::GroupControl => {
                self.registry
                    .matching_tokens_for_group_control(event.watched_address)
                    .await?
            }
            _ => {
        ...
        // Group messages have no meaningful on-chain sender/receiver to display (the real
        // sender is only recoverable after the client decrypts the payload), so skip the
        // address resolution the other event kinds rely on for display text.
        let sender = if matches!(event.kind, PushEventKind::GroupMessage) {
        ...
        let watched_addr = if matches!(event.kind, PushEventKind::GroupMessage) {
            "group".to_string()
        } else {
            to_rpc_address(&event.watched_address, self.network_type)?
...
struct PushPayload {
    ...
    timestamp: u64,
    daa_score: u64,
    /// Only set for `group_message` - the sender's per-member blinded group id (hex), so the
    /// client can identify which locally-known group this belongs to without an on-chain
    /// sender address to key off (see `PushEventKind::GroupMessage`'s doc comment).
    #[serde(skip_serializing_if = "Option::is_none")]
...
fn token_from_watched_group_key_bytes(key: &[u8]) -> Option<String> {
    const BLINDED_GROUP_ID_LEN: usize = 32;
    if key.len() <= BLINDED_GROUP_ID_LEN {
        return None;
    }
    let token_bytes = &key[BLINDED_GROUP_ID_LEN..];
    if !token_bytes.iter().all(|b| b.is_ascii_hexdigit()) {
        return None;
    }
    let token = std::str::from_utf8(token_bytes).ok()?;
...
fn decode_group_id_hex(value: &str) -> anyhow::Result<[u8; 32]> {
    ...
    if bytes.len() != 32 {
        anyhow::bail!("blinded_group_id must be 32-byte hex");
    }
    let mut out = [0u8; 32];
...
fn decode_hex_vec(value: &str) -> anyhow::Result<Vec<u8>> {
    ...
    while index < bytes.len() {
        let hi = (bytes[index] as char)
            .to_digit(16)
            .ok_or_else(|| anyhow::anyhow!("hex value contains non-hex characters"))?;
...
fn normalize_group_ids(ids: Vec<String>) -> anyhow::Result<(Vec<String>, Vec<[u8; 32]>)> {
    let mut seen = HashSet::new();
    let mut normalized = Vec::new();
    let mut bytes_out = Vec::new();
    for id in ids {
        let trimmed = id.trim().to_ascii_lowercase();
        let bytes = decode_group_id_hex(&trimmed)
            .map_err(|err| anyhow::anyhow!("Invalid watched_group_id: {err}"))?;
        if seen.insert(trimmed.clone()) {
            normalized.push(trimmed);
            bytes_out.push(bytes);
        }
    }
    Ok((normalized, bytes_out))
}
...
mod tests {
    ...
    use indexer_db::push::{
        DeviceRegistrationPartition, WatchedAddressPartition, WatchedGroupIdPartition,
    };
    ...
    fn validate_registration_limits_rejects_large_vectors() {
        ...
        assert!(
            validate_registration_limits(&addresses, &Vec::<String>::new(), &Vec::<String>::new())
                .is_err()
        );
        assert!(
            validate_registration_limits(&Vec::<String>::new(), &Vec::<String>::new(), &aliases)
                .is_err()
        );
    }
    ...
    async fn registry_actor_serializes_registration_and_filtering() {
        ...
        let registry = PushRegistry::new(
            ...
            WatchedGroupIdPartition::new(&tx_keyspace).expect("watched group partition"),
